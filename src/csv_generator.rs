...
impl CsvGenerator {
    pub fn from_config(config: &Config) -> Result<CsvGenerator> {
        ...
        let file_writer =
            TimestampFileWriter::new(config.csv_output_filename_pattern.clone(), Local::now());
    ...
    pub fn generate(&mut self, orig_data_set: &DataSet, now: &DateTime<UTC>) -> Result<()> {
        ...
        let need_header = if is_new_file { true } else { id_hash_differs };

        if need_header {
            debug!(
                "Needs header: is new file = {}, ID hash differs = {}, filename = {}",
                is_new_file,
                id_hash_differs,
