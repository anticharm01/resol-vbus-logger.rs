...
impl SqliteLogger {
    pub fn from_config(config: &Config) -> Result<SqliteLogger> {
        ...
        let mode = match (
            &config.sqlite_datasets_table,
            &config.sqlite_fields_table,
            &config.sqlite_statement,
            &config.sqlite_fields,
        ) {
            ...
            _ => {
                return Err(anyhow!(
                    "Unsupported combination of SQLlite logger configuration",
                ))
            }
        };
        ...
        match &self.mode {
            Mode::Relational {
                ...
                if stmt.next()? != State::Row {
                    return Err(anyhow!("Expected statement to return row"));
                }
